...
class EnemyGroup {
    ...
    addEnemies() {
        ...
        const groupHorizontalPosition = this.calculateRowOffset(maxLengthOfEnemyRows);
        const consoleTable = [];
        levelData.enemies.forEach((row, rowIndex) => {
            ...
            const maxWidthOfRow = row.length * Enemy.WIDTH + (row.length - 1) * this.ENEMY_SPACING;
            ...
            row.forEach((col, colIndex) => {
                ...
                const horizontalPosition = colIndex * (Enemy.WIDTH + this.ENEMY_SPACING) + horizontalOffset;
                ...
                const upperBoundSpace = ((maxLengthOfEnemyRows - row.length) / 2) * Enemy.WIDTH +
                ...
                const enemy = new Enemy(verticalPosition, horizontalPosition, minBound, 
